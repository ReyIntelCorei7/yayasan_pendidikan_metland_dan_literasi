...
function getText(val: any, lang: string): string {
  ...
  if (typeof val === 'object') {
    return val[lang] || val['id'] || val['en'] || Object.values(val)[0]?.toString() || '';
  }
  return String(val);
}
  ...
  return (
    <>
      ...
      <section className="relative h-[60vh] min-h-[400px] flex items-end bg-charcoal overflow-hidden">
        <img src={program.image || ''} alt={getText(program.title, i18n.language)} className="absolute inset-0 w-full h-full object-cover opacity-50" />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8 pb-16 w-full">
          ...
          <WordReveal text={getText(program.title, i18n.language)} tag="h1" className="text-4xl lg:text-6xl font-light text-white mt-4" delay={0.2} />
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="text-white/70 mt-3 text-lg">
            {getText(program.tagline, i18n.language)}
          </motion.p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8 grid grid-cols-3 gap-8">
          {program.stats?.map((stat: any, i: number) => (
            <div key={i} className="text-center">
              ...
              <div className="text-sm text-white/50 mt-1">{getText(stat.label, i18n.language)}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-[#FCFCFC] py-24">
        <div className="max-w-3xl mx-auto px-6 lg:px-8">
          <ScrollReveal>
            <p className="text-gray-500 leading-relaxed text-lg">{getText(program.description, i18n.language)}</p>
              ...
              {related.map((p: any) => (
                <Link key={p.id} to={`/programs/${p.slug}`} className="group">
                  <div className="aspect-[4/3] overflow-hidden rounded-2xl relative">
                    <img src={p.image || ''} alt={getText(p.title, i18n.language)} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
                    <div className="absolute bottom-4 left-4 right-4">
                      <h3 className="text-white font-medium">{getText(p.title, i18n.language)}</h3>
                      <p className="text-white/60 text-sm mt-1">{getText(p.tagline, i18n.language)}</p>
                    </div>
                  </div>
                </Link>
              ))}
